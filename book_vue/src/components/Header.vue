    ...
    <div class="header_wrap">
      ...
      <div class="login">
        ...
        <div v-else class="login_item login_succ">
          {{ $store.state.userInfo.user_name?$store.state.userInfo.user_name:"无名" }}
          <!-- 登陆成功，个人信息列表 -->
          <div class="login_infoList">
            <div class="user_img" :style="styleVar"></div>
            <div class="user_operator"><p><span @click="$router.push({path:'/myaccount/setaccount'})">账号设置</span><span @click="handerExit">退出</span></p></div>
          </div>
        </div>
        
        <div class="register_item" @click="handerRegister">免费注册</div>
        <div class="header_about register_item" v-if="$store.state.homeBol">关于我们</div>
        <div class="header_about register_item" v-else @click="$router.push({path:'/'})">首页</div>
      </div>
    </div>
    <!-- 头部右边功能模块 -->
